...
import { Card, EmptyState, LinkButton, MuscleTag, SectionTitle, HeroCard } from '../components/ui'
    ...
    <div className="animate-rise mx-auto max-w-2xl">
      <HeroCard className="p-5 pt-7 text-center sm:p-7">
        ...
        <div className="mt-6 grid grid-cols-2 gap-2 text-left sm:grid-cols-4">
          {(
            [
              [<Clock size={16} key="c" />, 'Duration', formatDuration(durationMs(w))],
              aerobic > 0 && volume === 0
                ? [<Footprints size={16} key="w" />, 'Aerobic', `${aerobic} min`]
                : [<Weight size={16} key="w" />, 'Volume', formatVolume(volume, units)],
              [<ListChecks size={16} key="s" />, 'Sets', String(completedSetCount(w))],
              [<Medal size={16} key="m" />, 'Records', String(prs.length)],
            ] as [ReactNode, string, string][]
          ).map(([icon, k, v]) => (
            <div key={k} className="rounded-[18px] bg-white/10 p-3.5">
              <span className="flex items-center gap-1.5 text-xs font-medium text-on-hero-muted">
                {icon} {k}
              </span>
      ...
      {aerobic > 0 && (
        <Card className="mt-3 p-4">
          <div className="flex items-baseline justify-between gap-3">
            <span className="text-[15px] font-semibold">Aerobic this week</span>
      ...
      <section className="mt-8">
        ...
        {prs.length ? (
          ...
          <p className="rounded-[20px] bg-surface-2/70 p-5 text-center text-sm text-muted">
