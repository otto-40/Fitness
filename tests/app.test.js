...
async function main() {
  ...
  await test('dropping a set drops its tick with it',
    ...
    check('two ticks are stored',
      ...
      [1, 1]);
    check('two of three pips are filled',
      await page.locator('[data-id="mon-2"] .pip.filled').count(), 2);
    check('and the lift no longer reads complete',
      await page.locator('[data-id="mon-2"].done').count(), 0);
    check('nor does the day', await page.locator('#d-mon .day-check.on').count(), 0);
  });
